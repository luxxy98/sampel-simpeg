...
final class AbsenJenisController extends Controller
{
    ...
    {
        return $this->transactionService->handleWithTransactionOn('absensi', function () use ($request) {
    ...
    public function handleWithTransactionOn(string $connection, callable $callback): JsonResponse
    {
        try {
            return DB::connection($connection)->transaction(static fn() => $callback());
