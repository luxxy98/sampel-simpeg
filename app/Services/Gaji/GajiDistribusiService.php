...
final class GajiDistribusiService
{
    ...
    {
        $q = DB::table('gaji_distribusi as gd')
            ->leftJoin('gaji_periode as gp', 'gp.id_periode', '=', 'gd.id_periode')
            ->leftJoin('person_sdm as ps', 'ps.id_sdm', '=', 'gd.id_sdm')
            ->leftJoin('person as p', 'p.id_person', '=', 'ps.id_person')
            ->leftJoin('sdm_rekening as sr', 'sr.id_rekening', '=', 'gd.id_rekening')
            ->select([
                'gd.id_distribusi',
                'gd.id_periode',
    ...
    {
        $row = DB::table('gaji_distribusi as gd')
            ->leftJoin('gaji_periode as gp', 'gp.id_periode', '=', 'gd.id_periode')
            ->leftJoin('person_sdm as ps', 'ps.id_sdm', '=', 'gd.id_sdm')
            ->leftJoin('person as p', 'p.id_person', '=', 'ps.id_person')
    ...
    {
        return DB::table('gaji_trx as gt')
